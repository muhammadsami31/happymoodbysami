...
function _objectSpread(b) {
    for (var c = 1; c < arguments.length; c++) {
        var d = null != arguments[c] ? arguments[c] : {}
          , e = Object.keys(d);
        "function" === typeof Object.getOwnPropertySymbols && (e = e.concat(Object.getOwnPropertySymbols(d).filter(function(b) {
            return Object.getOwnPropertyDescriptor(d, b).enumerable
        })));
        e.forEach(function(c) {
            _defineProperty(b, c, d[c])
        })
    }
    return b
}
...
(function() {
    function b(a, c) {
        ...
        this.wrapperWidth = 0;
        this.track = null;
        this.startPosition = this.trackPosition = this.trackWidth = 0;
        this.items = [];
        this.itemWidth = 0;
        this.lastItem = this.firstItem = null;
        this.settings = _objectSpread({}, b.defaults, c);
        this.settings.selector = a;
        this.playInterval = null;
        this.isStart = this.isEnd = this.navDisabled = !1;
        d(this) && (this.settings.navs && k(this),
        this.settings.play && this.play(),
        f(this),
        h(this))
    }
    function c(a) {
        a.track.style.transition = "transform ".concat(a.settings.speed, "ms ease");
        a.track.style.transform = "translate3d(".concat(a.trackPosition, "px,0,0)");
        a.settings.play && a.play()
    }
    function d(a) {
        a.wrapper = document.querySelector(a.settings.selector);
        if (!a.wrapper)
            return !1;
        a.wrapper.classList.add("ec");
        a.wrapperWidth = a.wrapper.clientWidth;
        a.itemWidth = parseInt(a.wrapperWidth / a.settings.items);
        a.items = Array.from(a.wrapper.children);
        if (0 === a.items.length)
            return !1;
        a.items.forEach(function(b) {
            b.classList.add("ec__item");
            b.style.width = a.itemWidth + "px"
        });
        if (1 < a.items.length) {
            var b = a.items.slice(a.items.length - a.settings.items, a.items.length).map(function(a) {
                a = a.cloneNode(!0);
                a.classList.add("cloned");
                return a
            })
              , c = a.items.slice(0, a.settings.items).map(function(a) {
                a = a.cloneNode(!0);
                a.classList.add("cloned");
                return a
            });
            a.items = [].concat(_toConsumableArray(b), _toConsumableArray(a.items), _toConsumableArray(c))
        }
        a.track = document.createElement("div");
        a.track.className = "ec__track";
        a.trackWidth = a.itemWidth * a.items.length;
        a.track.style.width = a.trackWidth + "px";
        a.items.forEach(function(b) {
            return a.track.appendChild(b)
        });
        b = document.createElement("div");
        b.className = "ec__holder";
        b.appendChild(a.track);
    ...
    function e(a) {
        ...
        a.track.style.width = a.trackWidth + "px";
        a.items.forEach(function(b) {
            return (b.style.width = a.itemWidth + "px")
        })
    }
    function h(a) {
        ...
        a.track.addEventListener("transitionstart", function() {
            a.isEnd = Math.abs(a.trackPosition) === a.trackWidth - a.itemWidth * a.settings.step
        });
        a.track.addEventListener("transitionend", function() {
            1 !== a.items.length && ((a.isEnd = Math.abs(a.trackPosition) === a.trackWidth - a.itemWidth * a.settings.items),
            a.isEnd && f(a),
            (a.isStart = 0 === Math.abs(a.trackPosition)),
            a.isStart && ((a.endPosition = Math.abs(a.trackPosition) - a.itemWidth * (2 * a.settings.items + 1)),
            (a.trackPosition = a.endPosition),
            g(a)));
            a.navDisabled = !1
        })
    }
    function k(a) {
        var b, c;
        b = a.settings.prevNav || document.createElement("div");
        b.className = "ec__nav ec__nav--prev";
        b.setAttribute("role", "button");
        b.setAttribute("disabled", "true");
        b.setAttribute("aria-label", "arrow");
        b.addEventListener("click", function() {
            a.navDisabled || ((a.navDisabled = !0),
            a.prev())
        });
        c = a.settings.nextNav || document.createElement("div");
        c.className = "ec__nav ec__nav--next";
        c.setAttribute("role", "button");
        c.setAttribute("aria-label", "arrow");
    ...
    function f(a) {
        1 === a.items.length ? (a.trackPosition = 0) : ((a.startPosition = (a.settings.start + a.settings.items) * a.itemWidth),
        (a.trackPosition = -1 * a.startPosition));
        g(a)
    }
    function g(a) {
        a.track.style.transitionProperty = "none";
        a.track.style.transform = "translate3d(".concat(a.trackPosition, "px,0,0)")
    }
    b.prototype.next = function() {
        (!this.settings.loop && this.isEnd) || 1 === this.items.length || ((this.trackPosition -= this.itemWidth * this.settings.step),
        c(this))
    }
    ;
    b.prototype.prev = function() {
        this.isStart = 0 === Math.abs(this.trackPosition);
        (!this.settings.loop && this.isStart) || 1 === this.items.length || ((this.trackPosition += this.itemWidth * this.settings.step),
        c(this))
    }
    ;
    b.prototype.goTo = function(a) {
        this.trackPosition = -(a * this.itemWidth);
        c(this)
    }
    ;
    ...
    window.ElderCarousel = b
}
)();
$(function() {

    $(".youtube").append("<iframe  src='https://www.youtube.com/embed/" + $("#video1").attr("video") + "?enablejsapi=1&origin=https://happymodpro.com&widgetid=1' frameborder=0 title='video review'></iframe>");

    $(".logo-search-input").click(function() {
        stop_search_keyword();
    });
    search_keyword();
    get_use_cookie();
    $(".use_cookie").click(function() {
        hide_use_cookie();
    });
    $(".dlink").click(function() {
        window.location.href = dlink;
    });
    $("#mail").attr("href", "mai" + "lto:sup" + "port@ha" + "ppymod.com");
    ...
    $(".download-bottom .close_box").click(function() {
        $(".download-bottom").hide();
    });
    if (document.body.clientWidth > 800) {
        $(".appie-traffic-title").addClass("on");
    }
    $(window).scroll(function() {
        if ($(window).scrollTop() > 600) {
            hide_use_cookie();
            $(".back-to-top").removeClass("hide");
            $(".download-bottom").removeClass("hide")
        } else if ($(window).scrollTop() < 550) {
            $(".back-to-top").addClass("hide");
            $(".download-bottom").addClass("hide")
        }
    });
    $(".menu_click").click(function(e) {
        $(".ocvs_overlay").addClass("active");
        $(".ocvs_menu_wrapper").addClass("active");
        stopPropagation(e);
    });
    $(".canvas_close").click(function() {
        $(".ocvs_overlay").removeClass("active");
        $(".ocvs_menu_wrapper").removeClass("active");
    });
    $(document).bind('click', function() {
        $(".ocvs_overlay").removeClass("active");
        $(".ocvs_menu_wrapper").removeClass("active");
    });
    $(".ocvs_menu_wrapper").click(function(e) {
        stopPropagation(e);
    });

    new ElderCarousel(".carousel-single-item",{
        items: 1,
        infinite: false
    });
    ...
    function resizeHandler2() {
        var width = window.innerWidth || document.documentElement.clientWidth;
        if (width < 576) {
            elem.settings.items = 1;
            videoelem2.settings.items = 1
        } else if (width < 768) {
            elem.settings.items = 2;
            videoelem2.settings.items = 2
        } else if (width < 992) {
            elem.settings.items = 3;
            videoelem2.settings.items = 3
        } else {
            elem.settings.items = 4;
            videoelem2.settings.items = 4
        }
    }
    let count = 4;
    let videocount2 = 4;
    resizeHandler();
    const elem = new ElderCarousel(".carousel-article-wrapper",{
        items: count,
        infinite: true
    });
    const videoelem2 = new ElderCarousel(".carousel-article-video",{
        items: videocount2,
        infinite: true
    });
    window.addEventListener('resize', throttle(resizeHandler, 1000));
    window.addEventListener('resize', throttle(resizeHandler2, 1000));
...
class LiteYTEmbed extends HTMLElement {
    connectedCallback() {
        ...
        this.videoImage = this.getAttribute('videoimage');
...
var search_int, keyword_gif, skip = 0;
function search_keyword() {
    var ki = 0;
    search_int = setInterval(function() {
        skip++;
        if (skip % 3 == 1) {
            ki = Math.floor(Math.random() * (keywords.length - 1));
            var left_str = keywords[ki] + ' ...';
            $(".logo-search-input").attr("value", "");
            keyword_gif = setInterval(function() {
                if (left_str != '') {
                    $(".logo-search-input").attr("value", $(".logo-search-input").attr("value") + left_str.substring(0, 1));
                    left_str = left_str.substring(1)
                } else {
                    clearInterval(keyword_gif)
                }
            }, 30)
        }
    }, 1000)
}
function stop_search_keyword() {
    clearInterval(search_int);
    $(".logo-search-input").attr("value", "");
}
function fix_keyword() {
    $(".logo-search-input").attr("value", $(".logo-search-input").attr("value").replace(" ...", ""));
    return true;
}
function hide_use_cookie() {
    $(".use_cookie").hide();
    var date = new Date();
    date.setTime(date.getTime() + 86400 * 1000 * 365);
    document.cookie = "use_cookie_ok=1; expires=" + date.toGMTString() + "; path=/";
}
function get_use_cookie() {
    if (document.cookie.indexOf("cookie_ok") < 0) {
        $(".use_cookie").removeClass("hide");
    }
}
